...
router = (name) => {
    ...
    const getPage = (pageName) => {
        console.log("GET PAGE:", _name,_baseRoute, pageName, __dirname)
        var pagePath = path.join(_baseRoute, pageName)
        console.log("page path", pagePath)
        if(pagePath.startsWith('home'))pagePath = '/'+pagePath
    ...
    const fileRouter = (file, variables) => {
        ...
        if (extention == 'html') {
            return function routesToRenderedHtml(request, response) {
                console.log("in router variables",variables)
                var html = fs.readFileSync(filePath).toString()

                //links to subpages look like ^{}
                var htmlWithSubPages = getSubPages(interpolate(html, variables || {}))
